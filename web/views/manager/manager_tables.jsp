...
<script>
    ...
    // Xử lý bật hiển thị Modal chi tiết kèm phân phối nút hành động tự động
    function handleTableClick(tableId, tableName, currentStatus, capacity) {
        ...
        // 1. Đồng bộ đường dẫn nhúng dữ liệu vào QR cho Customer quét
        ...
        // 2. Gọi API hiển thị ảnh QR động dạng phân giải cao
        ...
        // 4. Lấy danh sách các nút bấm điều khiển
        ...
        // Ẩn tất cả các nút biến đổi trước khi thực hiện phân loại
        ...
        // 5. Kiểm tra logic phân bổ giao diện theo 3 nhánh trạng thái
        if (statusNorm === 'empty' || statusNorm === 'trống') {
            ...
            btnQuickOpen.classList.remove('hidden'); // Hiện nút Mở bàn
        } 
            ...
            btnCancelRes.classList.remove('hidden'); // Hiện nút Hủy lịch đặt
        } 
        else {
            // Trường hợp: Occupied / Serving / Đang phục vụ / Đang ngồi
            ...
            btnResetTable.classList.remove('hidden'); // Hiện nút Dọn bàn về trống
            btnViewOrder.classList.remove('hidden');  // Hiện nút Xem Order hóa đơn
        }
    ...
    // [Hành động]: Gửi lệnh Mở bàn nhanh
function executeQuickOpen() {
    if(activeSelectedTableId !== null && activeSelectedTableId !== undefined && confirm("Xác nhận mở bàn này cho khách ngồi trực tiếp?")) {
        submitHiddenForm('quickOpen', activeSelectedTableId);
    }
}

// [Hành động mới 1]: Hủy lịch hẹn đặt trước của bàn đưa về trạng thái trống
function executeCancelReservation() {
    if(activeSelectedTableId !== null && activeSelectedTableId !== undefined && confirm("Bạn có chắc chắn muốn HỦY LỊCH HẸN và đưa bàn này quay về trạng thái TRỐNG không?")) {
        submitHiddenForm('cancelReservation', activeSelectedTableId);
    }
}

// [Hành động mới 2]: Trả bàn đang ngồi phục vụ về trạng thái Trống trống (Dọn bàn)
function executeResetTable() {
    if(activeSelectedTableId !== null && activeSelectedTableId !== undefined && confirm("Xác nhận hành động: Khách đã thanh toán rời đi, dọn bàn và ĐƯA BÀN VỀ TRẠNG THÁI TRỐNG?")) {
        submitHiddenForm('resetTable', activeSelectedTableId);
    }
}

    // Hàm bổ trợ đóng gói dữ liệu submit form ẩn lên Back-end Servlet an toàn
    ...
    // Điều hướng nhanh qua trang quản lý Order
    ...
    // Mở tab độc lập thực hiện lệnh in mã QR cho nhà hàng
    function printQRCode() {
        if(!activeSelectedQRUrl) return;
        const printWindow = window.open('', '_blank', 'width=400,height=400');
        printWindow.document.write('<html><head><title>In Mã QR Bàn</title>');
        printWindow.document.write('<style>body{display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;margin:0;font-family:sans-serif;text-align:center;} img{width:220px;height:220px;} h2{margin-bottom:5px;color:#006064;} p{font-size:12px;color:#666;margin-top:2px;}</style>');
        printWindow.document.write('</head><body>');
        printWindow.document.write('<h2>' + document.getElementById('modalTableTitle').innerText.replace("Quản lý", "Mã QR") + '</h2>');
        printWindow.document.write('<p>Quét mã để xem menu và gọi món</p>');
        printWindow.document.write('<img src="' + document.getElementById('modalQRImage').src + '" />');
        printWindow.document.write('<p style="font-weight:bold;margin-top:15px;">COFFEE POS SYSTEM</p>');

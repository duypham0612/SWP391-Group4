/* ============================================================================
   ...
   FILE SQL DUY NHẤT (đã gộp mọi migration + seed) — chạy 1 phát là xong:
     ...
     PART B            : catalog 15 món (đủ công thức) + ảnh Unsplash.
...
GO
IF SCHEMA_ID('catalog')   IS NULL EXEC('CREATE SCHEMA catalog');    -- menu, công thức (Admin)
GO
...
/* ===========================================================================
   2. CATALOG: danh mục, sản phẩm, nguyên liệu, công thức  (Admin)
   =========================================================================== */
...
CREATE TABLE sales.OrderItem (
    ...
    Size        VARCHAR(1) NOT NULL DEFAULT 'M' CHECK (Size IN ('S','M','L')),
    IceLevel    NVARCHAR(20) NOT NULL DEFAULT N'Bình thường',
    SugarLevel  VARCHAR(5) NOT NULL DEFAULT '100%',
    Note        NVARCHAR(255) NULL,
...
/* ############################################################################
   PART B — CATALOG ĐẦY ĐỦ (menu 15 món, mọi món đủ công thức + ảnh)
...
/* --- B4. Publish menu CN01 cho toàn bộ món (idempotent) ------------------- */
...
DECLARE @voucherSeed TABLE(
    Code NVARCHAR(40), DiscountType VARCHAR(10), DiscountValue DECIMAL(10,2), MinOrderAmount DECIMAL(12,2),
    Scope VARCHAR(10), BranchId INT NULL, StartDate DATETIME2, EndDate DATETIME2, UsageLimit INT, IsActive BIT
);
INSERT INTO @voucherSeed(Code, DiscountType, DiscountValue, MinOrderAmount, Scope, BranchId, StartDate, EndDate, UsageLimit, IsActive) VALUES
 ('CHAIN10',      'PERCENT',10,     0,'CHAIN', NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,3,@now),3000,1),
 ('CHAIN15',      'PERCENT',15, 50000,'CHAIN', NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),2000,1),
 ('CHAIN20',      'PERCENT',20,100000,'CHAIN', NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,1,@now),1500,1),
 ('CAFE25',       'PERCENT',25,150000,'CHAIN', NULL,DATEADD(DAY,-1,@now),DATEADD(DAY,45,@now), 1000,1),
 ('MORNING12',    'PERCENT',12,     0,'CHAIN', NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),2500,1),
 ('WEEKEND18',    'PERCENT',18, 80000,'CHAIN', NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),1800,1),
 ('HAPPY30K',     'FIXED',30000,150000,'CHAIN',NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),1200,1),
 ('DON80K',       'FIXED',10000, 80000,'CHAIN',NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,3,@now),3500,1),
 ('DON120K',      'FIXED',20000,120000,'CHAIN',NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,3,@now),2500,1),
 ('COMBO50K',     'FIXED',50000,250000,'CHAIN',NULL,DATEADD(DAY,-1,@now),DATEADD(MONTH,1,@now), 800,1),
 ('Q1WELCOME15',  'PERCENT',15,     0,'BRANCH',@b1, DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),1000,1),
 ('Q1MORNING10',  'PERCENT',10, 30000,'BRANCH',@b1, DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),1200,1),
 ('Q1FIX20K',     'FIXED',20000,100000,'BRANCH',@b1,DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now), 900,1),
 ('TDSTUDENT20',  'PERCENT',20, 50000,'BRANCH',@b2, DATEADD(DAY,-1,@now),DATEADD(MONTH,3,@now),1500,1),
 ('TDLUNCH15',    'PERCENT',15, 70000,'BRANCH',@b2, DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),1000,1),
 ('TDFIX25K',     'FIXED',25000,120000,'BRANCH',@b2,DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now), 900,1),
 ('HCWELCOME15',  'PERCENT',15,     0,'BRANCH',@b3, DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),1000,1),
 ('HCAFTERNOON12','PERCENT',12, 40000,'BRANCH',@b3, DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now),1200,1),
 ('HCFIX20K',     'FIXED',20000,100000,'BRANCH',@b3,DATEADD(DAY,-1,@now),DATEADD(MONTH,2,@now), 900,1),
 ('HCWEEKEND20',  'PERCENT',20, 90000,'BRANCH',@b3, DATEADD(DAY,-1,@now),DATEADD(MONTH,1,@now), 700,1);
INSERT INTO payment.Voucher(Code,DiscountType,DiscountValue,MinOrderAmount,Scope,BranchId,StartDate,EndDate,UsageLimit,IsActive)
SELECT s.Code,s.DiscountType,s.DiscountValue,s.MinOrderAmount,s.Scope,s.BranchId,s.StartDate,s.EndDate,s.UsageLimit,s.IsActive
FROM @voucherSeed s
WHERE NOT EXISTS(SELECT 1 FROM payment.Voucher v WHERE v.Code=s.Code);

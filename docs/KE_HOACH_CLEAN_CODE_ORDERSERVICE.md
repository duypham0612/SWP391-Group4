...
| 2026-08-03 | 25 file `service/**` | **`repository.tx` KHÔNG phải đường chung.** Đếm thật: 25 service tự mở transaction bằng `setAutoCommit(false)` thay vì đi qua helper. Nên mọi thứ gắn vào `tx` (thử lại deadlock, luật rollback RuntimeException) đều KHÔNG tự động áp dụng cho chúng. Đây là lý do bản sửa deadlock đầu tiên hụt. | 🟢 **XONG (Đợt 7, §5e)** — đã gom toàn bộ 84 khối về `config/Tx`. `setAutoCommit` nay chỉ còn ở đúng `config/Tx.java` trong cả dự án |
| 2026-08-03 | `dao/**` (7 lớp) | **Code chết có sẵn, KHÔNG do Đợt 3.** Quét toàn repo sau khi bỏ facade: `BillDao.countPaidToday`/`findStatusesByTable`, `OrderItemDao.findBrewedToday`, `PrepBatchDao.updateQuantity`, `RecipeDao.deleteByOwner`/`exists`/`findByOwnerIds`/`findDepletedProductIds`/`findModifierLinesByGroup`, `ShiftAssignmentDao.findByBranchRange`/`findOpenByUserAndBranch` — 0 caller. Đã đối chiếu tại `HEAD` trước Đợt 3: cũng 0 caller, tức không phải hồi quy. | 🟢 **XONG** — đã xoá 9/11. Hai cái còn lại KHÔNG chết: `RecipeDao.findByOwnerIds` và `ShiftAssignmentDao.findByBranchRange` được gọi bởi method public khác **trong chính file đó** (bộ dò ban đầu chỉ đếm caller ngoài `dao/` nên báo nhầm — xoá thử đã làm gãy biên dịch) |

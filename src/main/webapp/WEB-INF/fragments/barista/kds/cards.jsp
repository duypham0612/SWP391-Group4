...
<c:if test="${board.peakMode}">
    <div class="kds-peak" role="status">
        <strong>Cao điểm</strong>
        <span>${board.peakQueueCups} ly đang dồn · pha lần lượt theo số thứ tự đầu dòng</span>
    </div>
</c:if>
...
<span id="kdsQueueState" hidden
      data-page="${board.queuePage.page}" data-total-pages="${board.queuePage.totalPages}"
      data-owner="${board.filterOwner}" data-station="${board.filterStation}" data-order-type="${board.filterOrderType}"></span>

<c:choose>
    <c:when test="${board.queueTotal == 0}">
        <div class="kds-tables-empty"><span>✓</span> Quầy đang thông thoáng — chưa có món nào cần pha.</div>
    </c:when>
    <c:when test="${board.queuePage.total == 0}">
        <div class="kds-filter-empty">Không có món phù hợp bộ lọc — quầy vẫn còn ${board.queueTotal} món.</div>
    </c:when>
    <c:otherwise>
        <div class="kds-queue" id="kdsQueue">
            ...
            <c:forEach var="item" items="${board.queuePage.items}">
                <c:set var="cardItem" value="${item}" scope="request" />
        <jsp:include page="queue-row.jsp" />
            </c:forEach>
        </div>
            ...
            <c:if test="${board.queuePage.totalPages gt 1}">
                <nav class="pagination" aria-label="Phân trang hàng chờ pha chế">
                    <a class="page" href="${kdsPagerBase}1" data-page="1"
                       aria-disabled="${not board.queuePage.hasPrevious}" title="Trang đầu">«</a>
                    <a class="page" href="${kdsPagerBase}${board.queuePage.page - 1}" data-page="${board.queuePage.page - 1}"
                       aria-disabled="${not board.queuePage.hasPrevious}" title="Trang trước">‹</a>
                    <c:forEach var="pageNumber" items="${board.queuePage.visiblePages}">
                        <a class="page ${pageNumber == board.queuePage.page ? 'is-active' : ''}"
                           href="${kdsPagerBase}${pageNumber}" data-page="${pageNumber}"
                           aria-current="${pageNumber == board.queuePage.page ? 'page' : 'false'}">${pageNumber}</a>
                    </c:forEach>
                    <a class="page" href="${kdsPagerBase}${board.queuePage.page + 1}" data-page="${board.queuePage.page + 1}"
                       aria-disabled="${not board.queuePage.hasNext}" title="Trang sau">›</a>
                    <a class="page" href="${kdsPagerBase}${board.queuePage.totalPages}" data-page="${board.queuePage.totalPages}"
                       aria-disabled="${not board.queuePage.hasNext}" title="Trang cuối">»</a>
                </nav>
            </c:if>
        </div>
    </c:otherwise>

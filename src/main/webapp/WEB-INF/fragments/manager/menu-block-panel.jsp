    ...
    <c:choose>
        ...
        <c:otherwise><div class="table-scroll"><table class="table">
            ...
            <tbody><c:forEach var="r" items="${openRequests}"><tr>
                <td><strong>${r.productName}</strong></td><td>${view.menuReason(r.reason)}</td>
                ...
                <td><span class="${r.overdue ? 'badge badge-cancelled' : ''}">${view.shortUtc(r.backInEta)}</span><c:if test="${r.overdue}"><small class="muted" style="display:block;color:var(--st-cancelled)">Đã quá thời gian dự kiến</small></c:if></td>
                <td>${r.requesterName}<small class="muted" style="display:block">${view.shortUtc(r.requestedAt)}</small></td>

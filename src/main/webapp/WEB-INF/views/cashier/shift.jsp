        ...
        <div>
            ...
            <c:if test="${not empty clockStatus and clockStatus.hasAssignment}">
                <div class="shift-clock-meta">
                    ...
                    <span>${view.shiftStatus(clockStatus)}</span>
                </div>
            </c:if>
        </div>
        <c:choose>
...
<c:choose>
    ...
    <c:when test="${dutyState == 'ON_DUTY' and not empty current}">
        <div class="card form-card">
            ...
            <p>Mở lúc ${view.fullUtc(current.openedAt)} · Quỹ đầu ca <strong><fmt:formatNumber value="${current.openingCash}" maxFractionDigits="0"/> ₫</strong></p>
                ...
                <tbody>
                    <c:forEach var="s" items="${history}">
                        <tr>
                            ...
                            <td>${s.cashierName}</td>
                            <td>${view.fullUtc(s.openedAt)}</td>
                            <td><c:choose><c:when test="${s.open}"><span class="badge badge-making">Đang mở</span></c:when><c:otherwise>${view.fullUtc(s.closedAt)}</c:otherwise></c:choose></td>

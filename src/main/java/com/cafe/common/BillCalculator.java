 ...
 *
 * Quy ước: giá dòng đã được snapshot vào OrderItem.BilledAmount lúc tạo bill. Bill:
 *   subtotal  = Σ amount các dòng trên bill
 *   discount  = giảm giá nhập thủ công, kẹp trong [0, subtotal]
...
public final class BillCalculator {
    ...
    /** Chuẩn hoá giảm giá thủ công, không cho âm hoặc vượt subtotal. */
    public static BigDecimal clampDiscount(BigDecimal value, BigDecimal subtotal) {
        if (value == null || subtotal == null) return BigDecimal.ZERO.setScale(2);
        BigDecimal d = value;

...
class UserServiceIT extends SqlServerIntegrationSupport {
    ...
    void create_user_through_service_can_authenticate() throws Exception {
        ...
        int roleId = roleId("BARISTA");
        String username = unique("staff");
        String password = "MvcTest!123";

        int userId = new UserService().createUser(user(username, roleId, branchId), password);
    ...
    void create_user_translates_duplicate_username_email_and_phone() throws Exception {
        ...
        User first = user(unique("duplicate"), roleId, branchId);
        service.createUser(first, "MvcTest!123");

        User sameUsername = user(first.getUsername(), roleId, branchId);
        ...
        User sameEmail = user(unique("email"), roleId, branchId);
        ...
        User samePhone = user(unique("phone"), roleId, branchId);
    ...
    void concurrent_manager_creation_allows_exactly_one_manager_per_branch() throws Exception {
        ...
        int managerRoleId = roleId("BRANCH_MANAGER");
        User first = user(unique("manager"), managerRoleId, branchId);
        User second = user(unique("manager"), managerRoleId, branchId);
        ...
        try {
            ...
            assertEquals(1, scalarInt(
                    "SELECT COUNT(*) FROM iam.UserAccount WHERE BranchId=? AND RoleId=?",
                    branchId, managerRoleId));
    ...
    void update_user_rechecks_admin_invariant_in_service() throws Exception {
        int branchId = scalarInt("SELECT TOP (1) BranchId FROM org.Branch WHERE IsActive=1 ORDER BY BranchId");
        UserService service = new UserService();
        int userId = service.createUser(user(unique("update"), roleId("BARISTA"), branchId), "MvcTest!123");
        User update = service.getUser(userId);
        update.setRoleId(roleId("ADMIN"));
    ...
    private User user(String username, int roleId, int branchId) {
        User user = new User();
    ...
    private int roleId(String code) throws Exception {
        return scalarInt("SELECT RoleId FROM iam.Role WHERE Code=?", code);
    }
